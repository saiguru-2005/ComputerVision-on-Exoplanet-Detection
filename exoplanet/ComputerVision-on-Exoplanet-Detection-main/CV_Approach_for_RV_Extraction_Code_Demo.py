...
fitsFile_folder = r'C:\Users\saine\OneDrive\Documents\Sai guru\ComputerVision-on-Exoplanet-Detection\exoplanet\ComputerVision-on-Exoplanet-Detection-main\EXPRES\101501\Spectra 101501'

if not os.path.exists(fitsFile_folder):
    print(f"Directory does not exist: {fitsFile_folder}")
else:
    ...
    for folder in folders:
        ...
        print(f"Directories were created successfully at path: {dir_path}")

    print("\r\n**********************************************************************************************************************")
    print("\r\n                    A Computer Vision Approach to Radial Velocity Extraction for Exoplanet Detection                  ")
    print("                                                     Demo Code                                                            ")
    print("                                        Author: (saigurcharank@gmail.com)                                   ")
    print("\r\n**********************************************************************************************************************")

    # Add the rest of your script logic here
    

    # Example processing logic (assuming you have these variables defined)
    Avg_Spectrum_OverSampling = 2  # Example value
    n_start = 1  # Example value
    n_stop = 60  # Example value

    for order in range(n_start, n_stop + 1):
        print(f"Working on Order #{order}")

    # Ensure variables used in the filename are defined
    filename = DataSet + 'templates_ao' + str(Avg_Spectrum_OverSampling) + '_n' + str(n_start) + '_n' + str(n_stop) + '.pickle'
    print(f"Saving to file: {filename}")

    # Save to pickle (assuming you have some data to save)
    data_to_save = {"example_key": "example_value"}  # Replace with actual data
    with open(filename, 'wb') as f:
        pickle.dump(data_to_save, f)

    # Add this code before line 769 to handle the CSV file
    #activity_csv_path = "C:\Users\saine\OneDrive\Documents\Sai guru\Exoplanet\exoplanet\ComputerVision-on-Exoplanet-Detection-main\EXPRES\101501\101501_activity.csv"
    activity_csv_path = r"C:\Users\saine\OneDrive\Documents\Sai guru\Exoplanet\exoplanet\ComputerVision-on-Exoplanet-Detection-main\EXPRES\101501\101501_activity.csv"

    if not os.path.exists(activity_csv_path):
        print(f"File does not exist: {activity_csv_path}")
    else:
        X = pd.read_csv(activity_csv_path)
...
for n_start, n_stop in n_pairs: # loop on n_pairs
    ...
    for Avg_Spectrum_OverSampling in Avg_Spectrum_OverSamplings:
        ...
        widthMax = 12 * Avg_Spectrum_OverSampling
        AvgSpectrum = (
            'AvgSpectrum_with_Chunks_ao' + str(Avg_Spectrum_OverSampling) +
            '_nMin' + str(n_start) + '_nMax' + str(n_stop) + '_cutoff' + 
            str(cutoffRatio) + '_widthMax' + str(widthMax) + '_heightMin' + 
            str(heightminRatio) + '_' + Chunk_Algorithm_Version
        )
        for v_Pixel in v_Pixels:
            ...
            for w_oversampling in w_oversamplings:
                ...
                savetoFolder = os.path.join(
                    RawSpectrumImg_folder, 
                    AvgSpectrum + '_v' + str(v_Pixel) + '_w' + '_OS' + str(w_oversampling) +
                    '_uncrty' + str(uncertainty) + '_grayscale'
                )
...
data_file = EXPRES_folder + DataSet+Star+'101501_activity.csv'
...
data_file_path = r'C:\Users\saine\OneDrive\Documents\Sai guru\ComputerVision-on-Exoplanet-Detection\exoplanet\ComputerVision-on-Exoplanet-Detection-main\EXPRES\101501\101501_activity.csv'


# List contents of the dataset directory for debugging
dataset_dir = os.path.dirname(data_file_path)
print(f"Listing contents of the dataset directory for debugging:\n{os.listdir(dataset_dir)}")

# Path to the pickle file
pickle_file_path = os.path.join(dataset_dir, 'Spectra 101501.pickle')
print(f"Pickle file found: {pickle_file_path}")

# Load data from pickle file
with open(pickle_file_path, 'rb') as f:
    data = pickle.load(f)

# Ensure the expected number of items
expected_num_items = 45
if len(data) != expected_num_items:
    raise ValueError(f"Unexpected number of items in pickle file: {len(data)}")

# Print detailed structure of the pickle file
for i, item in enumerate(data):
    print(f"Item {i}: {type(item)} - shape: {item.shape if isinstance(item, np.ndarray) else 'N/A'}")

# Example unpacking: adjust this based on what your data represents
# This example assumes the first 6 items have specific roles and the rest are additional data
try:
    totTemplate = data[0]
    wavelength = data[1]
    shift_per_pixel = data[2]
    chunkIndices = data[3]
    start_pixel_array = data[4]
    stop_pixel_array = data[5]
    additional_data = data[6:]

    # Further processing of loaded data...
    print("totTemplate:", totTemplate)
    print("wavelength:", wavelength)
    print("shift_per_pixel:", shift_per_pixel)
    print("chunkIndices:", chunkIndices)
    print("start_pixel_array:", start_pixel_array)
    print("stop_pixel_array:", stop_pixel_array)

    # Further steps to calculate and plot RVs (Radial Velocities)
    # Note: Add your specific calculation and plotting code here

    # Example plotting (replace with your actual plotting logic)
    plt.figure(figsize=(10, 6))
    plt.plot(wavelength, totTemplate)
    ...
    print("Please check the structure of your pickle file and adjust the unpacking logic accordingly.")
...
def create_pickle_from_fits(fits_files, output_pickle):
    data = []
    for fits_file in fits_files:
        with fits.open(fits_file) as hdul:
            data.append(hdul[0].data)  # Adjust this based on the structure of your FITS files
...
# Define the correct folder paths
EXPRES_folder =r"C:\Users\saine\OneDrive\Documents\Sai guru\ComputerVision-on-Exoplanet-Detection\exoplanet\ComputerVision-on-Exoplanet-Detection-main\EXPRES"
 
#"C:/Users/Rakesh/OneDrive/Documents/Exoplanet/exoplanet/ComputerVision-on-Exoplanet-Detection-main/EXPRES/"
DataSet = "101501/"
Star = "101501"
...
# Load, plot, and save the activity csv
data_file = os.path.join(EXPRES_folder, DataSet, f"{Star}_activity.csv")
print(f"Data file path: {data_file}")
X = pd.read_csv(data_file)

# get a list of all directories under a DataSet
RV_Results_folder = os.path.join(EXPRES_folder, DataSet)  # Assuming RV_Results_folder is within DataSet directory
RVPickle_folders = [os.path.join(RV_Results_folder, name) for name in os.listdir(RV_Results_folder) if os.path.isdir(os.path.join(RV_Results_folder, name))]

print("Listing contents of the dataset directory for debugging:")
print(os.listdir(RV_Results_folder))

# Check if the required pickle file exists
required_pickle_file = os.path.join(EXPRES_folder, DataSet, 'Spectra 101501.pickle')
if not os.path.exists(required_pickle_file):
    print(f"File not found: {required_pickle_file}")
    fits_folder = os.path.join(EXPRES_folder, DataSet, 'Spectra 101501')
    fits_files = [os.path.join(fits_folder, f) for f in os.listdir(fits_folder) if f.endswith('.fits')]
    if fits_files:
        create_pickle_from_fits(fits_files, required_pickle_file)
        print(f"Created pickle file at: {required_pickle_file}")
    else:
        print(f"No FITS files found in {fits_folder}. Cannot create pickle file.")
else:
    print(f"Pickle file found: {required_pickle_file}")

for RVPickle_folder in RVPickle_folders:    
    # List all pickle files in the RVPickle_folder for debugging
    print(f"Contents of {RVPickle_folder}:")
    print(os.listdir(RVPickle_folder))

    # extract avererage spectrum chunk info file location
    AvgSpectrum_Chunks_File_Name = os.path.basename(RVPickle_folder)
    # load the shift_per_pixel data
    AvgSpectrum_Chunks_File = os.path.join(EXPRES_folder, DataSet, f"{AvgSpectrum_Chunks_File_Name}.pickle")
    print(f"AvgSpectrum_Chunks_File path: {AvgSpectrum_Chunks_File}")
    
    if os.path.exists(AvgSpectrum_Chunks_File):
        # Inspect the pickle file to understand its content
        data = inspect_pickle_file(AvgSpectrum_Chunks_File)
        
        # Unpack based on the actual content
        if len(data) == 6:
            totTemplate, wavelength, shift_per_pixel, chunkIndices, start_pixel_array, stop_pixel_array = data
        elif len(data) == 5:
            totTemplate, wavelength, shift_per_pixel, chunkIndices, start_pixel_array = data
            stop_pixel_array = None  # or handle appropriately
        else:
            raise ValueError(f"Unexpected number of items in pickle file: {len(data)}")
        
        del totTemplate, wavelength, chunkIndices, start_pixel_array, stop_pixel_array

        os.makedirs(os.path.join(RVPickle_folder, 'RV_Plots'), exist_ok=True)  # make a directory to save plots
        PickleFiles = [p for p in os.listdir(RVPickle_folder) if p.endswith('.pickle')]  # Get a list of all the pickle files in the folder

        # check if there is a saved RV_data file
        RV_summary_file = os.path.join(RVPickle_folder, 'RV_summary.pickle')
        if os.path.exists(RV_summary_file): # there is a saved RV_data file
            with open(RV_summary_file, 'rb') as f:
                RV_data = pickle.load(f)    
        else:
            RV_data = [] # No previously saved RV_data, start with an empty array
        ...
        for RVPickleFile in PickleFiles:
            if RVPickleFile != 'RV_summary.pickle':
                ...
                avgRV = calculateRV(os.path.join(RVPickle_folder, RVPickleFile), RV_cutoff, w_oversampling, n_start, n_stop, shift_per_pixel)
                ...
                if not np.isnan(avgRV[:,1]).any(): # all RV data are good
                    ...
                    plt.ylim([yl_low, yl_high])
                    plt.tight_layout()
                    savetofile = os.path.join(RVPickle_folder, 'RV_Plots', f"{RV_info}_RVRMS{str(round(RV_RMS, 3))}_dot.png")
                    fig.savefig(savetofile)
                    ...
                    plt.ylim([yl_low, yl_high])
                    plt.tight_layout()
                    savetofile = os.path.join(RVPickle_folder, 'RV_Plots', f"{RV_info}_RVRMS{str(round(RV_RMS, 3))}_line.png")
                    fig.savefig(savetofile)
        ...
        print(f"File not found: {AvgSpectrum_Chunks_File}")
